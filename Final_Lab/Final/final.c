...
void hitBottomReact(struct boid* flock, int bottom_wall) {
  ...
  flock->y = int2fix5(bottom_wall - 1);
}
...
void positionUpdate(struct boid* flock, int i)
{
  ...
  if (flock[i].x + flock[i].y >= int2fix5(640)){
    ...
    } else if (int2fix5(160) < flock[i].x && flock[i].x <= int2fix5(280)){
    ...
    } else if(int2fix5(280) < flock[i].x && flock[i].x <= int2fix5(400)){
    ...
    } else if(int2fix5(400) < flock[i].x && flock[i].x <= int2fix5(520)){
      int right_wall = 519;
      int bottom_wall = 240;
      if (hitBottom(flock[i].y , bottom_wall) && hitRight(flock[i].x + flock[i].vx, right_wall)) {
        hit_flag = 1;
